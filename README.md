...
each other, why they are shaped that way) is written once, by the
maintainer, in a small Python module. Build state is derived, never
...
## Quick start
    ...
    # edit atlas/model.py: name your components, place them, write the sentences
    uv run systemap refresh
    open docs/atlas/index.html

Commit `docs/atlas/`, and to publish it enable GitHub Pages from the `docs/`
...
- `Component`: one card. `kind` is `component` (does work), `store` (holds
  ...
  56, 52 or 44 tall by kind. `implemented_by` names modules, `entry` the
  function or class that makes the component real. `tracker` marks a
  roadmap item whose code has not landed (an issue number in it, `#12`,
  becomes a link through `issue_url`); `note` is a caveat the reader sees.
...
| `model` | `atlas/model.py` | the module exporting `MODEL` and `MEANING` |
| `out_dir` | `docs/atlas` | where the facts, the page and the figures go |
...
## Commands
    ...
    systemap init                 write systemap.toml, atlas/model.py, docs/atlas/.gitkeep,
    ...
    systemap check                layout, routes, labels, type size, meaning, wheels
...
Each failure names the fix. Commit `docs/atlas/` after `systemap refresh`;
the diff of the facts file between two commits is a readable record of what
changed about the system.

## How kstrl uses it

`examples/kstrl/` is a complete model: 54 components in 9 regions and 6
containers, 60 flows with a sentence each, 7 layers, 4 journeys, 15
invariants, and a theme. `examples/kstrl/systemap.toml` shows every
configuration key in use, including two figures the design documents cite.
Its facts file is committed beside it so `systemap check` and
`systemap render` run from that directory without the kstrl tree.
...
not cards, components are, and a component is a thing the maintainer
decided exists. It is not a UML tool: there is one diagram type, one
layout the maintainer places by hand, and no notation to learn beyond
card, line and label. The meaning is written by a person on purpose,
because the one thing the code cannot tell you is why a part is shaped the
way it is.
