...
- Artifact labels sit on their edges. A label is a noun phrase of
  ...
  applies, from the router's own seat counts: `gutter between rows 2 and
  3 holds 3 of 3 seats: move a card or widen the row pitch` (the room
  across the gutter is used up) or `label is 41 units wider than its
  seat: shorten the artifact` (no run of the path is long enough for the
  words).
...
first, and the gutter its label sits in; every gutter with the seats used
at its fullest point of the seats it has (a seat is one label across the
gutter, 13 units with a 2-unit gap and 3 units clear of the cards); and
