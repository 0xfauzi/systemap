---
name: systemap
description: Map a repository with systemap. Use when asked to map a system, draw a system map, write or update map/model.py, group modules into components, or make `systemap check` pass. Runs the extractor, drafts the model, checks it until every module is mapped, renders the page, and hands the maintainer the list of judgement calls to confirm.
---
