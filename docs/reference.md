...
| `systemap journeys [--limit N] [--dry-run]` | write a walk through the system for a way in that no journey starts from: the agent named under `[agent] command` reads the code from that way in and answers with the cards a run passes through and a sentence each; a step tracing a flow the map does not draw is refused and printed as a line to fix, not written; what holds is written into the model marked `drafted=True`, which `judgement` prints as a `drafted journey` line until you read it and remove the mark; three walks a run by default; `--dry-run` lists the ways in and writes nothing; with no `[agent] command` it lists them and says so |
